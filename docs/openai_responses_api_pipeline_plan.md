...
| **4**       | **Persist Tool Metadata**                     | 1. Update tool execution code so `tool_calls` and `tool_responses` are stored in the chat DB. <br> 2. Remove any embedded JSON approach if used; rely on database fields for replays.                                                             | Done |                   |
