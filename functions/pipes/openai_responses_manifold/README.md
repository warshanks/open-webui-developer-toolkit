...
Embedding full OpenAI response metadata directly into the message text significantly increases storage overhead. Instead, encoding a concise ID inside an empty link optimizes storage while enabling complete metadata retrieval from the database.
