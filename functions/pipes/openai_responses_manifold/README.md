...
| Response item persistence | ✅ GA | 2025-06-16 | Stores function calls and other non-message items using hidden Markdown link IDs. |
...
```
Here, each hidden identifier is embedded invisibly as an empty Markdown link.

On subsequent API calls:

1. The pipeline decodes the hidden Markdown link identifiers embedded in the messages.
...
Embedding full OpenAI response metadata directly into hidden links significantly increases storage overhead. Instead, encoding a concise, unique identifier optimizes storage while enabling complete metadata retrieval from the database.
...
* We immediately insert `[](01HX4Y2VW5VR2Z2HDQ5QY9REHB)` and yield it so it's permanently embedded into `body["messages"]["content"]`.
